...
sender3(void)
{
  ...
  while (true)
  {
    s.kind = data;              /* data frame for transmission */
    ...
    if (event == frame_arrival)
    {
      ...
      /* else if (s.ack != (1 -next_frame_to_send)) re-send */
    }
...
receiver3(void)
{
  ...
  while (true)
  {
    ...
    if (event == frame_arrival) /* A valid frame has arrived. */
    {
      ...
      s.kind = ack; /* sending an ACK */ 
      s.ack = frame_expected; /* ACK, ask for frame_expected */
