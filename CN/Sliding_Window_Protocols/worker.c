 ...
 */
  ...
  if (nseqs < 0)
    nseqs = oldest_frame; /* need MAX_SEQ+1 for protocol 6 */
 ...
 */
  ...
  if (fstat(prfd, &statbuf) < 0)
    sim_error("Cannot fstat peer pipe");
  ...
  if (frct > 0)
  {
    ...
    if (k > frct)
      k = frct; /* how many frames to read from peer */
 ...
 */
  ...
  case 6:
	 ...
	 */
    if (s->kind == data)
      seqs[s->seq % (nseqs / 2)] = s->seq; /* save seq # */
  }
