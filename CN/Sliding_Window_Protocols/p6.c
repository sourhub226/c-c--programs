/* Protocol 6 (selective repeat) accepts frames out of order, 
   but passes packets to the network layer in order. 
   Associated with each outstanding frame is a timer. When the timer
   goes off, only that frame is retransmitted, 
   not all the outstanding frames, as in protocol 5. 
 */
